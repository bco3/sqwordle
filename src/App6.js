...
export const App6 = () => {
  ...
  // const defaultWord6 =
  //   targetWords6[Math.floor(Math.random() * targetWords6.length)];
  ...
  // const [tiles6, setTiles6] = useState(defaultTiles6);
  const [keyRows, setKeyRows] = useState(defaultKeyRows);
  const [word, setWord] = useState(defaultWord);
  // const [word6, setWord6] = useState(defaultWord6);
  ...
  //   hiScore30: 0,
  // };
  // const [stats6, setStats6] = useState(defaultStats6);
  ...
  //   streak30: 0,
  // };
  // const [streaks6, setStreaks6] = useState(defaultStreaks6);
  ...
  useEffect(() => {
    localForage.getItem("tiles6").then((value) => {
      setTiles(value || defaultTiles);
    });
  }, []);
  useEffect(() => {
    hasMounted && saveData("tiles6", tiles);
  }, [tiles]);

  useEffect(() => {
    localForage.getItem("keyRows6").then((value) => {
      setKeyRows(value || defaultKeyRows);
    });
  }, []);
  useEffect(() => {
    hasMounted && localForage.setItem("keyRows6", keyRows);
  }, [keyRows]);

  useEffect(() => {
    localForage.getItem("word6").then((value) => {
      setWord(value || defaultWord);
    });
  }, []);
  useEffect(() => {
    hasMounted && saveData("word6", word);
  }, [word, hasMounted]);

  useEffect(() => {
    localForage.getItem("stats6").then((value) => {
      setStats(value || defaultStats);
    });
  }, []);
  useEffect(() => {
    hasMounted && saveData("stats6", stats);
  }, [stats]);

  useEffect(() => {
    localForage.getItem("streaks6").then((value) => {
      setStreaks(value || defaultStreaks);
    });
  }, []);
  useEffect(() => {
    hasMounted && saveData("streaks6", streaks);
  }, [streaks]);

  useEffect(() => {
    localForage.getItem("todaysGameSwitch6").then((value) => {
      setTodaysGameSwitch(value || defaultTodaysGameSwitch);
    });
  }, []);
  useEffect(() => {
    hasMounted && saveData("todaysGameSwitch6", todaysGameSwitch);
    hasMounted && resetGame();
  }, [todaysGameSwitch]);

  useEffect(() => {
    localForage.getItem("timerDelay6").then((value) => {
      setTimerDelay(value || defaultTimerDelay);
    });
  }, []);
  useEffect(() => {
    hasMounted && saveData("timerDelay6", timerDelay);
  }, [timerDelay]);

  useEffect(() => {
    localForage.getItem("timerSwitch6").then((value) => {
      setTimerSwitch(value || defaultTimerSwitch);
    });
  }, []);
  useEffect(() => {
    hasMounted && saveData("timerSwitch6", timerSwitch);
  }, [timerSwitch]);

  useEffect(() => {
    localForage.getItem("position6").then((value) => {
      setPosition(value || defaultPosition);
    });
  }, []);
  useEffect(() => {
    hasMounted && saveData("position6", position);
  }, [position]);

  useEffect(() => {
    localForage.getItem("data6").then((value) => {
      setData(value || defaultData);
    });
  }, []);
  useEffect(() => {
    hasMounted && saveData("data6", data);
  }, [data]);
    ...
    for (let i = position.row; i >= 0; i--) {
      ...
      newTilesWin[i].forEach((_, index) => {
        if (index % 2 !== 0) {
          ...
          }, 3000 + animationDuration * i + (animationDuration / 2) * index);
        }
      });
    }
    for (let i = position.row; i >= 0; i--) {
      ...
      newTilesWin[i].forEach((_, index) => {
        if (index % 2 !== 0) {
          setTimeout(() => {
            ...
            i === 0 && index === 11 && resetGame(0) && console.log("has reset");
            return setTiles([...newTilesWin]);
  ...
  function randomNewGameResponse() {
  ...
  function comicBubbleInOut(delay) {
    ...
    // const squirtleTalk = squirtleTalkRef.current;
    gsap.set(".squirtle-talk", {
      transform: "scale(0) translateX(-50%)",
      zIndex: 0,
    }); // Set the initial position of the squirtleTalk
    // Create the elastic transition
    const tween = gsap.to(".squirtle-talk", {
      duration: 1,
      delay: 0.5,
      transform: "scale(1) translateX(-50%)",
      zIndex: 30,
      ease: "elastic.out(1, 0.6)", // Use the elastic easing function
      paused: true, //Pause the animation initially
    });
    // Start the animation after 5 seconds and revert back to the original position
    setTimeout(() => {
      tween.reverse();
    }, delay);
    tween.play(); // Start the animation
  }
  function squirtleInOut(delay) {
    // const squirtleAnimation = squirtleAnimationRef.current;
    gsap.killTweensOf(".squirtle");
    gsap.set(".squirtle", { bottom: "-15em" }); // Set the initial position of the squirtleAnimation
    // Create the elastic transition
    const tween = gsap.to(".squirtle", {
      duration: 1.5,
      bottom: "-1em",
      ease: "elastic.out(1, 0.4)", // Use the elastic easing function
      paused: true, // Pause the animation initially
    });
    // Start the animation after 5 seconds and revert back to the original position
    setTimeout(() => {
  ...
  useEffect(() => {
    function handleKeyPress(e) {
      ...
      if (e.key.match(/^[a-z]$/)) {
        console.log(`position from key is now ${position.letterPosition}`);
        pressKey(e.key);
        return;
      }
    }
  ...
  function pressKey(key) {
    if (position.letterPosition >= 11) return;
    let newTiles = [...tiles];
    newTiles[position.row][position.letterPosition] = key.toUpperCase();
  ...
  function submitGuess() {
    ...
    if (!dictionary.includes(guess.toLowerCase())) {
      ...
      setSubtitles(randomWrongResponse());
      const newSquirtleStatus = squirtleStatus === "mad" ? "mad2" : "mad";
      setSquirtleStatus(newSquirtleStatus);
      setComicBubbleDirection("talk-right");
      comicBubbleInOut(1500);
      squirtleInOut(1500);
      shakeTiles();
  ...
  function flipTile(tile, index, array, guess) {
    const key = () => {
      let letter = tile.toUpperCase();
      for (var i = 0; i < keyRows.length; i++) {
        for (var j = 0; j < keyRows[i].length; j = j + 2) {
          if (keyRows[i][j] === letter) {
            return [i, j];
          }
        }
      }
    ...
    setTimeout(() => {
      ...
      if (word[index / 2] === tile) {
        ...
        let newKeysTested = [...keyRows];
        newKeysTested[keyPosition[0]][keyPosition[1] + 1] =
          keyRows[keyPosition[0]][keyPosition[1] + 1] + " wrong-location";
        setKeyRows([...newKeysTested]);
      } else {
        let newTilesTested = [...tiles];
        newTilesTested[position.row][index + 1] =
        ...
        let newKeysTested = [...keyRows];
        newKeysTested[keyPosition[0]][keyPosition[1] + 1] =
          keyRows[keyPosition[0]][keyPosition[1] + 1] + " wrong";
        setKeyRows([...newKeysTested]);
      }
    ...
    }, (index * animationDuration * 2) / 3 + animationDuration * 2);
  }
  ...
  function wordOfDay() {
    ...
    return targetDailyWords[offSetByGameCount()].toUpperCase();
  }
  ...
  function danceTiles() {
    tiles[position.row].forEach((_, index) => {
      ...
      if (index % 2 !== 0) {
        newTilesShake[position.row][index] =
          newTilesShake[position.row][index] + " dance";
        return setTiles([...newTilesShake]);
      }
    });
    setTimeout(() => {
    ...
    <div className="container">
      ...
      <div className="under-header-container">
        <div className="stats" style={{ top: showStats ? "5em" : "-19em" }}>
          ...
          >
            <div className="stat-header">YOUR 6 WORD STATS</div>
        ...
        <div data-guess-grid className="guess-grid6">
          ...
          {tiles.map((_, tileRow) => {
            return tiles[tileRow].map((_, tile) => {
              return tile % 2 === 0 ? (
                <div
                  className={`bubble${bubbleMix[tileRow][tile]} tile  ${
                    tiles[tileRow][tile + 1]
                  }`}
                  key={"row" + tileRow + "letter" + tile}
                >
                  {tiles[tileRow][tile]}
                </div>
              ) : undefined;
        ...
        <div data-keyboard className="keyboard">
          ...
          {keyRows[0].map((letter, index) => {
            return index % 2 === 0 ? (
              <button
                className={keyRows[0][index + 1]}
                data-key={letter}
                onClick={(e) => {
                  handleClick(e.target);
                  pressKey(letter);
                }}
                key={"row top letter " + letter}
              >
                {keyRows[0][index]}
              </button>
            ) : undefined;
          ...
          {keyRows[1].map((letter, index) => {
            return index % 2 === 0 ? (
              <button
                className={keyRows[1][index + 1]}
                data-key={letter}
                onClick={(e) => {
                  handleClick(e.target);
                  pressKey(letter);
                }}
                key={"row middle letter " + letter}
              >
                {letter}
              </button>
            ) : undefined;
          ...
          {keyRows[2].map((letter, index) => {
            return index % 2 === 0 ? (
              <button
                className={keyRows[2][index + 1]}
                data-key={letter}
                onClick={(e) => {
                  handleClick(e.target);
                  pressKey(letter);
                }}
                key={"row bottom letter " + letter}
              >
                {letter}
              </button>
            ) : undefined;
